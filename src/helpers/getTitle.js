...
export default async function getTitle(id) {
  ...
  if (!props?.aboveTheFoldData) {
    throw new Error("Invalid or missing IMDb data structure");
  }
  ...
  return {
    ...
    releaseDetailed: {
      ...
      releaseLocation: {
        country:
          props.mainColumnData?.releaseDate?.country?.text ?? null,
    ...
    filmingLocations:
      props.mainColumnData?.filmingLocations?.edges?.map(
        (e) => e.node?.text
      ) ?? [],
    actors: getCredits("cast"),
    actors_v2: getCredits("cast", "2"),
